...
namespace Mercer.Tpa.Agenda.Web.UI.BusquedaCitas
{
    ...
    public partial class ControlFiltroBusquedas : UserControl
    {
        ...
        private string _selectedEmpresas;
        ...
        private ParametrosBusquedaCita GetParametrosBusqueda()
        {
            ...
            //RAM* Asignacion nuevas empresas
            //parametros.idEmpresa = SessionManager.IdEmpresa;
            parametros.idEmpresa = _selectedEmpresas;
        ...
        public void AsignarFiltros()
        {
            ...
            if (DropEmpresas.SelectedIndex > 0)
            {
                _selectedEmpresas = "";
                _selectedEmpresas = Convert.ToString(DropEmpresas.SelectedValue);
            }
            if (DropEmpresas.SelectedIndex == 0)
            {
                _selectedEmpresas = "";
                for (int i = 1; i < DropEmpresas.Items.Count; i++)
                {
                    _selectedEmpresas += Convert.ToString(DropEmpresas.Items[i].Value) + ",";
                }
                _selectedEmpresas = _selectedEmpresas.Substring(0, _selectedEmpresas.Length - 1);
            }
        ...
        private void CargarEmpresas()
        {
            try
            {
                ...
                DropEmpresas.DataSource = user.GetEmpresasUserCorporativo(SessionManager.IdUser, user.GetCorporativoEmpresa(SessionManager.IdEmpresa));
