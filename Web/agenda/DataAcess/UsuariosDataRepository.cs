...
namespace Mercer.Tpa.Agenda.Web.DataAcess
{
    ...
    public class UsuariosDataRepository
    {
        ...
        //RAM
        internal object GetEmpresasUserCorporativo(int idUsuario, int idCorporativo)
        {
            DataSet ds = new DataSet();
            try
            {
                ds = SqlHelper.ExecuteDataset(_strCon, "[dbo].[ListEmpresasUserCorporativo]", idUsuario, idCorporativo);
                return ds;
            }
            catch (Exception ex)
            {
        ...
        //RAM
        internal int GetCorporativoEmpresa(int idEmpresa)
        {
            int ds;
            try
            {
                ds = (int)SqlHelper.ExecuteScalar(_strCon, "[dbo].[GetCorporativoEmpresa]", idEmpresa);
                return ds;
            }
            catch (Exception ex)
            {
