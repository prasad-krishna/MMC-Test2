...
{
    /// <summary>
    /// Descripción breve de AE_liquidarreembolso.
    /// </summary>
    public partial class AE_liquidarreembolsos : PB_PaginaBase
    {
        #region Atributos
        ...
        public ArrayList getEstadoSolicitud()
        {
            ...
            if (Convert.ToInt16(this.ddlEstados.SelectedValue) == Convert.ToInt16(Solicitud.EnumEstadoSolicitud.Liquidado))
            {
                ...
                this.dtgSolicitudes.Columns[13].Visible = false;
                this.dtgSolicitudes.Columns[14].Visible = true;

                this.dtgSolicitudes.Columns[16].Visible = false; // PETF 14/01/10
            }
            ...
            if (Convert.ToInt16(this.ddlEstados.SelectedValue) == Convert.ToInt16(Solicitud.EnumEstadoSolicitud.Negado) || Convert.ToInt16(this.ddlEstados.SelectedValue) == Convert.ToInt16(Solicitud.EnumEstadoSolicitud.Anulado))
            {
                if (Convert.ToInt16(this.ddlEstados.SelectedValue) == Convert.ToInt16(Solicitud.EnumEstadoSolicitud.Anulado))
                {
                    ...
                    this.dtgSolicitudes.Columns[16].Visible = true; // PETF 14/01/10
                }
                else
                {
                    this.dtgSolicitudes.Columns[12].Visible = false;
        ...
        protected void ddlEstados_SelectedIndexChanged(object sender, System.EventArgs e)
        {
            try
            {
                ...
                if (Convert.ToInt32(Session["Company"]) == 85 && Convert.ToInt16(this.ddlEstados.SelectedValue) == Convert.ToInt16(Solicitud.EnumEstadoSolicitud.Liquidado))
                {
                    this.lblFechaConfirmacion.Visible = true;
                    this.txtFechaConfirmacion.Visible = true;
                    this.imgCalendario.Style["display"] = "";
                    this.rfvFechaConfirmacion.Enabled = true;
                }
                else
                {
                    this.lblFechaConfirmacion.Visible = false;
                    this.txtFechaConfirmacion.Visible = false;
            ...
            try
            {
                if (e.CommandName == "Procesar")
                {
                    ...
                    if (Convert.ToInt16(this.ddlEstados.SelectedValue) == Convert.ToInt16(Solicitud.EnumEstadoSolicitud.Liquidado))
                    {
                        ...
                        if (Session["SICAU"] != null)
                            objSolicitud.Usuario_idLiquidacion = Convert.ToInt32(Session["IdUser"]);
                        else
                            objSolicitud.IdUserLiquidacion = Convert.ToInt32(Session["IdUser"]);
                    }
                    //Si se va a liquidar se carga el id de la solicitud de SICAU para cierre
                    if (Convert.ToInt16(this.ddlEstados.SelectedValue) == Convert.ToInt16(Solicitud.EnumEstadoSolicitud.Anulado))
                    {
                        ...
                        {
                            objSolicitud.ObservacionAnulacion = txtObservacionesAnulacion.Text;                            
                        }
                    }
                ...
                if (e.CommandName == "Ver")
                {
                    ...
                    this.OpenWindow("AE_SolicitudReembolso.aspx?IdSolicitud=" + e.Item.Cells[0].Text + "&employee_id=" + e.Item.Cells[2].Text + "&popup=1", 900, 700, Convert.ToInt32(ViewState["Ver"]));
                }
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Evento, Procesar masivamente cambios de estado 
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
            ...
            foreach (DataGridItem item in this.dtgSolicitudes.Items)
            {
                if (item.ItemType == ListItemType.Item || item.ItemType == ListItemType.AlternatingItem)
                {
                    ...
                    if (chkProcesar.Checked)
                    {
                        ...
                        objTipoServicio.IdSolicitudEstado = Convert.ToInt16(this.ddlEstados.SelectedValue);
                        objTipoServicio.FechaConfirmacion = new DateTime(1900, 1, 1);
                        if (this.txtFechaConfirmacion.Text != string.Empty)
